...
for i = 1 : 5
    disp(['Insert joint', num2str(i),' speed [m/s]',':'])
...
for i=1:length(t_out)
   ...
   % Recover Current Configuration
    ...
    C3_t(i,:)=rL(1:2,1)';   %set for convenience
    Oj3_t(i,:)=rJ(1:2,1)';    %set for convenience
    C4_t(i,:)=rL(1:2,2)';   %set for convenience
    Oj4_t(i,:)=rJ(1:2,2)';    %set for convenience
    C5_t(i,:)=rL(1:2,1)';   %set for convenience
    Oj5_t(i,:)=rJ(1:2,1)';    %set for convenience
   
    posEEwrtC1_L1=(RL(:,:,1)*[l(1);0;0])';
    posEEwrtC2_L2=(RL(:,:,2)*[l(2);0;0])';
    posEEwrtC3_L3=(RL(:,:,1)*[l(3);0;0])';
    posEEwrtC4_L4=(RL(:,:,2)*[l(4);0;0])';
    posEEwrtC5_L5=(RL(:,:,1)*[l(5);0;0])';
...
%     MoMt_OI2(i,:)= MoMt_C0(i,:)+((MBB+MLL2)*SkewSym(I_r_B)*dot_r_com)';
    OC=r_com;
    OC0=I_r_B;
